...
function ProjectsPlanet() {
    ...
    () =>
      new THREE.ShaderMaterial({
        ...
        uniforms: {
          uLightDir: { value: LIGHT_DIR },
          uTime: { value: 0 },
          uColHigh: { value: new THREE.Color("#00d4ff") },
  ...
  const atmoMat = useMemo(() => createAtmosphereMaterial("#00d4ff", 1.2), []);
...
function ContactSun() {
  ...
  useFrame((state) => {
    ...
    // Mission complete sequence: the sun flares when the mission completes
    const imp = scrollState.missionComplete;
